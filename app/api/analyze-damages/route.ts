  ...
  try {
    ...
    for (const photoFile of media) {
      try {
        ...
        const file = new File([buffer], photoFile.name, {
          type: photoFile.type,
        });
          ...
          content: [
            {
              type: "input_text",
              text: `
            You are an expert auto insurance damage assessor. You will be provided information about a vehicle and images of damages. Analyze the following vehicle accident and provide a detailed damage assessment in a JSON format.
...
${accident.description}
    ...
    if (!data) {
      return NextResponse.json(
        { error: "No response from prompt API" },
        { status: 500 }
      );
    }
    ...
    if (!result.success) {
      return NextResponse.json(
        { error: "Invalid response from OpenAI", details: result.error },
        { status: 500 }
      );
    }
